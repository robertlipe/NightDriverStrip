...
#endif
{
  private:
    ...
    // Locations of snowflakes stored in a single flattened bitset
    // (X-major). Index calculation: idx = y * MATRIX_WIDTH + x
    std::unique_ptr<std::bitset<MATRIX_WIDTH * MATRIX_HEIGHT>> snowBits =
        make_unique_psram<std::bitset<MATRIX_WIDTH * MATRIX_HEIGHT>>();
    ...
    // Accessor for snow bitfield: non-const returns a reference proxy for
    // read/write, const overload returns a bool for read-only contexts.
    inline std::bitset<MATRIX_WIDTH * MATRIX_HEIGHT>::reference snowAt(
        uint8_t x, uint8_t y)
    {
