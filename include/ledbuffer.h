...
class LEDBuffer
{
  ...
  public:
    ...
    bool UpdateFromWire(std::unique_ptr<uint8_t []> & payloadData, size_t payloadLength)
    {
        ...
        {
            debugW("command16: %d   length32: %ld,  payloadLength: %d\n", command16, length32, payloadLength);
        ...
        debugV("PayloadLength: %d, command16: %d, Length32: %ld", payloadLength, command16, length32);
        ...
        debugV("Color0: %08lx", (uint32_t) _leds[0]);
        return true;
    }
