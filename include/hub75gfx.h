...
class HUB75GFX : public GFXBase
{
  ...
  public:
        ...
        std::vector<std::shared_ptr<GFXBase>> &devices)
    {
        ...
        {
            auto tmp_matrix =
                make_shared_psram<HUB75GFX>(MATRIX_WIDTH, MATRIX_HEIGHT);
        ...
        std::static_pointer_cast<HUB75GFX>(devices[0])
    ...
    // Whereas an WS281xGFX would track its own memory for the CRGB array,
    // we simply point to the buffer already used for the matrix display
    // memory.  That also eliminated having a local draw buffer that is
    // then copied, because the effects can render directly to the right
    // back buffer automatically.
    ...
    void Clear(CRGB color = CRGB::Black) override
    {
        ...
        if (color.g == color.r && color.r == color.b)
        {
            ...
            memset((void *)backgroundLayer.backBuffer(), color.r,
                   sizeof(HUB75GFX::SM_RGB) * _ledcount);
        }
        else
        {
