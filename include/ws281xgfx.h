  ...
  protected:
    ...
    #define ADD_CHANNEL(channel)                                         \
        debugI("Adding %zu LEDs to pin %d from channel %d on FastLED.",  \
               devices[channel]->GetLEDCount(), LED_PIN##channel,        \
               channel);                                                 \
        FastLED                                                          \
    ...
    WS281xGFX(size_t w, size_t h) : GFXBase(w, h)
    {
        ...
        if (!leds)
            throw std::runtime_error(
                "Unable to allocate LEDs in WS281xGFX");
    }
...
// We don't support more than 8 parallel channels
    ...
    #error The maximum value of NUM_CHANNELS (number of parallel channels) is 8
        ...
        for (int i = 0; i < NUM_CHANNELS; i++)
        {
            ...
            devices.push_back(make_shared_psram<WS281xGFX>(
...
// HexagonGFX
//
// A version of the WS281xGFX class that accounts for the layout of the
// hexagon LEDs
//
