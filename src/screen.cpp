...
// Helper function to determine if we're on a small display that should
// skip headers/footers
inline bool IsSmallDisplay(const Screen &display)
{
    // Use actual screen dimensions - if height is less than 160px,
    // consider it small
    ...
    int ContentTop(Screen &display) const
    {
        if (IsSmallDisplay(display))
            return display.fontHeight() +
                   2; // Just one line for effect name on small displays
    ...
    void Draw(Screen &display, bool bRedraw) override
    {
        ...
        if (IsSmallDisplay(display))
        {
                ...
                millis() - lastFullDraw > 1000)
            {
                ...
                {
                    display.fillRect(0, 0, display.width(), _contentTop,
                                     backColor);
                    ...
                    display.setTextColor(display.GetTextColor(),
                                         backColor);
                    String effectName = g_ptrSystem->EffectManager()
    ...
    // Add spectrum analyzer content
    #if ENABLE_AUDIO
        {
            ...
            const int bottomMargin =
                IsSmallDisplay(display) ? 0 : display.BottomMargin;
            ...
            const int bandHeight =
                display.height() - spectrumTop - bottomMargin;
...
class EffectSimulatorPage final : public TitlePage
{
  ...
  public:
    EffectSimulatorPage()
    {
        g_ptrSystem->EffectManager().AddFrameEventListener(
            frameEventListener);
    }
    ...
    void Draw(Screen &display, bool bRedraw) override
    {
        ...
        // Always update shared header/footer so stats (LED/Aud/Ser/Scr)
        // stay fresh
        TitlePage::Draw(display, bRedraw);

        // If no new effect frame, skip the matrix blit but keep the
        // refreshed footer. Also mark FPS as touched so the loop-based
        // fallback doesn't overwrite the effect-driven FPS with the UI
        // refresh cadence.
        ...
        const int contentHeight =
            display.height() - bottomMargin - contentTop;
        const int contentWidth = display.width();

        // Matrix dimensions - handle single-row LED strips by wrapping
        // into a matrix
        int mw = MATRIX_WIDTH;
        int mh = MATRIX_HEIGHT;

        // If we have a single row of LEDs (strip), wrap it into a matrix
        // for better visualization
        if (mh == 1 && mw > 32) // Only wrap if we have more than 32 LEDs
                                // in a single row
        {
            // Calculate optimal wrapping based on available screen space
            int targetRows =
                contentHeight / 3; // Aim for ~3 pixel high rows
            if (targetRows < 4)
        ...
        // Blit: draw each LED as a scale x scale rectangle (direct buffer
        // reads, no per-dest-pixel loop) Track effect draw cadence and
        // update the screen FPS to reflect actual effect rendering rate
...
void IRAM_ATTR Screen::Update(bool bRedraw)
{
    ...
    // If the active page did not set FPS explicitly, fall back to generic
    // frame cadence
